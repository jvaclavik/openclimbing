...
// Renders an accumulated-precipitation map (24h / 48h / 72h) by summing the raw ČHMÚ
...
const ALLOWED_HOURS = new Set([24, 48, 72]);
const TS_RE = /^\d{8}\.\d{4}$/;
...
export default async function handler(
  ...
  if (!ALLOWED_HOURS.has(hours)) {
    res.status(400).end('Invalid hours (24, 48 or 72)');
    return;
  }
